...
class STSN(AnomalyModule):
    ...
    def teacher_channel_mean_std(self, dataloader: DataLoader) -> dict[str, Tensor]:
        ...
        for batch in tqdm.tqdm(dataloader, desc="Calculate teacher channel mean", position=0, leave=True):
            y = self.model.teacher(batch["image"].to(self.device), batch["depth_image"].to(self.device))
    ...
    def map_norm_quantiles(self, dataloader: DataLoader) -> dict[str, Tensor]:
        ...
        for batch in tqdm.tqdm(dataloader, desc="Calculate Validation Dataset Quantiles", position=0, leave=True):
            ...
            for img, depth_img, label in zip(batch["image"], batch["depth_image"], batch["label"]):
                if label == 0:  # only use good images of validation set!
                    # output = self.model(img.to(self.device))
                    output = self.model(img.to(self.device), depth_img.to(self.device))
                    map_st_rgb = output["map_st_rgb"]
                    map_st_d = output["map_st_d"]
                    maps_st_rgb.append(map_st_rgb)
                    maps_st_d.append(map_st_d)
        qa_st_rgb, qb_st_rgb = self._get_quantiles_of_maps(maps_st_rgb)
        qa_st_d, qb_st_d = self._get_quantiles_of_maps(maps_st_d)
        return {"qa_st_rgb": qa_st_rgb, "qa_st_d": qa_st_d, "qb_st_rgb": qb_st_rgb, "qb_st_d": qb_st_d}
    ...
    def configure_optimizers(self) -> optim.Optimizer:
        optimizer = optim.Adam(
            list(self.model.student_rgb.parameters()) + list(self.model.student_d.parameters()),
            lr=self.lr,
    ...
    def training_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> dict[str, Tensor]:
        ...
        loss_st_rgb, loss_st_d, loss_st_rgbd = self.model(batch=batch["image"], batch_depth = batch["depth_image"], batch_imagenet=batch_imagenet)

        loss = loss_st_rgb + loss_st_d + loss_st_rgbd
        self.log("train_st_rgb", loss_st_rgb.item(), on_epoch=True, prog_bar=True, logger=True)
        self.log("train_st_d", loss_st_d.item(), on_epoch=True, prog_bar=True, logger=True)
        self.log("train_st_rgbd", loss_st_rgbd.item(), on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        batch["anomaly_maps"] = self.model(batch["image"], batch["depth_image"])["anomaly_map_combined"]
